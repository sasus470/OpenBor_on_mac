...
static void assign_first_joystick_to_player1(void)
{
    if(!usejoy || numjoy <= 0 || !joystick[0] || !player1_uses_default_keyboard())
    {
        return;
    }

    savedata.keys[0][SDID_MOVEUP] = JOY_LIST_FIRST + 1;
    savedata.keys[0][SDID_MOVERIGHT] = JOY_LIST_FIRST + 2;
    savedata.keys[0][SDID_MOVEDOWN] = JOY_LIST_FIRST + 3;
    savedata.keys[0][SDID_MOVELEFT] = JOY_LIST_FIRST + 4;
    savedata.keys[0][SDID_ATTACK] = JOY_LIST_FIRST + 5;
    savedata.keys[0][SDID_ATTACK2] = JOY_LIST_FIRST + 6;
    savedata.keys[0][SDID_ATTACK3] = JOY_LIST_FIRST + 7;
    savedata.keys[0][SDID_ATTACK4] = JOY_LIST_FIRST + 8;
    savedata.keys[0][SDID_JUMP] = JOY_LIST_FIRST + 9;
    savedata.keys[0][SDID_SPECIAL] = JOY_LIST_FIRST + 10;
    savedata.keys[0][SDID_START] = JOY_LIST_FIRST + 11;
    savedata.keys[0][SDID_SCREENSHOT] = JOY_LIST_FIRST + 12;
}
		...
		switch(ev.type)
		{
			case SDL_KEYDOWN:
				...
				if(lastkey == SDL_SCANCODE_F11 ||
				   ((keystate[SDL_SCANCODE_LALT] || keystate[SDL_SCANCODE_RALT]) && (lastkey == SDL_SCANCODE_RETURN)))
				{
					video_fullscreen_flip();
					keystate[SDL_SCANCODE_RETURN] = 0;
					keystate[SDL_SCANCODE_F11] = 0;
				}
				if(lastkey != SDL_SCANCODE_F10) break;
#else
				lastkey = ev.key.keysym.sym;
				if(lastkey == SDLK_F11 ||
				   ((keystate[SDLK_LALT] || keystate[SDLK_RALT]) && (lastkey == SDLK_RETURN)))
				{
					video_fullscreen_flip();
...
void control_init(int joy_enable)
{
    ...
    assign_first_joystick_to_player1();
...
int control_connected_joysticks()
{
    return numjoy;
}
